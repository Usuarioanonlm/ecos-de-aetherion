...
export default function AuthGate({ onAuthenticated }: { onAuthenticated: (email: string) => void }) {
  ...
  const submit = async () => { if (!email || password.length < 6) { setMessage("Informe um e-mail e senha de no mínimo 6 caracteres."); return; } const result = mode === "login" ? await supabase.auth.signInWithPassword({ email, password }) : await supabase.auth.signUp({ email, password, options: { emailRedirectTo: AUTH_REDIRECT_URL } }); if (result.error) { const text = readableError(result.error.message); setMessage(text); if (text.includes("Aguarde 1 minuto")) setSeconds(60); if (text.includes("confirmado")) setPendingConfirmation(true); return; } if (result.data.session?.user) { onAuthenticated(result.data.session.user.email ?? email); return; } setPendingConfirmation(true); setSeconds(60); setMessage("Conta criada. Confira a caixa de entrada e o spam para confirmar seu e-mail."); };
  const resend = async () => { if (!email || seconds) return; const result = await supabase.auth.resend({ type: "signup", email, options: { emailRedirectTo: AUTH_REDIRECT_URL } }); if (result.error) { const text = readableError(result.error.message); setMessage(text); if (text.includes("Aguarde 1 minuto")) setSeconds(60); return; } setMessage("Novo e-mail enviado. Aguarde até 1 minuto e confirme pelo link recebido."); setSeconds(60); };
  return <main className="auth-gate"><section><div className="auth-cartography" aria-hidden="true" /><header className="auth-brand"><span className="auth-seal" aria-hidden="true"><i /></span><div><p>REGISTRO DE AETHERION · CONTA</p><b>ECOS DE AETHERION</b></div></header><h1>{mode === "login" ? "RETORNE AO ÉTER" : "CRIE SEU ECO"}</h1><span>{mode === "login" ? "Apresente seu selo para retomar a trilha do seu Eco no PC ou no celular." : "Registre seu novo Eco para que a Ruptura guarde sua jornada em qualquer dispositivo."}</span><label>E-mail<input value={email} onChange={(event) => setEmail(event.target.value)} type="email" autoComplete="email" /></label><label>Senha<input value={password} onChange={(event) => setPassword(event.target.value)} type="password" autoComplete={mode === "login" ? "current-password" : "new-password"} /></label>{message && <small>{message}</small>}<button onClick={submit}>{mode === "login" ? "ENTRAR NO REGISTRO" : "SELAR CONTA"}</button>{pendingConfirmation && <button className="auth-resend" disabled={seconds > 0} onClick={resend}>{seconds > 0 ? `REENVIAR EM ${seconds}s` : "REENVIAR CONFIRMAÇÃO"}</button>}<button className="auth-switch" onClick={() => { setMode(mode === "login" ? "register" : "login"); setMessage(""); }}>{mode === "login" ? "Ainda não tem registro? Criar agora" : "Já possui um registro? Entrar"}</button></section></main>;
}
